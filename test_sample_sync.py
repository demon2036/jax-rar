...
def _nucleus_sampling(rng,logits ,p: float=0.95, t: float = 1.02, ):
  logits = logits / t
...
def sample( key,params,tokenizer_params, model,tokenizer_jax, config, batch_size=1,
    ...
    # key,key_prefill,key_decode,key_condition=jax.random.split(key[0],4)

    # condition = jax.random.randint(key_condition, (batch_size, 1), 0, 1000)
    ...
    # next_token=_nucleus_sampling(key_prefill,logits[:,-1])
...
def main3():
    ...
    for _ in tqdm.tqdm(range(iters)):
        ...
        sample_rng,sample_img=sample_jit(sample_rng,model_params,tokenizer_params)
        # print(sample_rng)
